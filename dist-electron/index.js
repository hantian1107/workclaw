...
class SessionManager {
   ...
   * @returns 清理的会话数量
   */
  cleanupExpiredSessions(maxAge = 24 * 60 * 60 * 1e3) {
    const now = Date.now();
    let cleanedCount = 0;
...
class ContextManager {
  ...
  pruneInactiveSessions(maxAgeMs = 24 * 60 * 60 * 1e3) {
...
class ConversationManager {
  ...
  getRecentMessages(sessionId, limit2 = 10) {
    const conversation = this.getConversation(sessionId);
    return conversation.slice(-limit2);
  }
  ...
  importConversation(sessionId, conversation) {
    this.conversations.set(sessionId, conversation);
  }
  getSessionIds() {
    return Array.from(this.conversations.keys());
  }
  pruneInactiveConversations(maxAgeMs = 24 * 60 * 60 * 1e3) {
    const now = (/* @__PURE__ */ new Date()).getTime();
...
class PromptManager {
  systemPrompt = `You are Workclaw, a helpful AI assistant running on the user's local machine. You can assist with various tasks such as file management, code analysis, and system operations. Always prioritize the user's privacy and security. Only access files and directories that the user has explicitly authorized.`;
  toolPromptTemplate = "Tool execution result:\n\n{toolResult}\n\nPlease summarize this result in a natural, friendly way for the user.";
...
class WorkspaceManager {
  ...
  hasAccess(path2, permission) {
    const currentWorkspace = this.getCurrentWorkspace();
    if (!currentWorkspace) {
      return false;
    }
    for (const resource of currentWorkspace.resources) {
      if (path2.startsWith(resource.path)) {
...
async function list(directoryPath, options) {
  return new Promise((resolve, reject) => {
...
class Core {
  ...
  async executeFileOperation(operation, params) {
    ...
    switch (operation) {
      case "read":
        return await this.sandbox.execute(() => read(path2, options));
      case "write":
        if (!this.sandbox.checkAccess(path2, "write")) {
          return "Error: Access denied to write file";
        }
        return await this.sandbox.execute(() => write(path2, content, options));
      case "delete":
        if (!this.sandbox.checkAccess(path2, "write")) {
          return "Error: Access denied to delete file";
        }
        return await this.sandbox.execute(() => deleteFile(path2));
      case "list":
        return await this.sandbox.execute(() => list(path2, options));
      default:
  ...
  async executeBrowserOperation(operation, params) {
    ...
    switch (operation) {
      case "open":
        return await this.sandbox.execute(() => open(url));
      case "getContent":
        return await this.sandbox.execute(() => getContent(url));
      default:
